...
class SettingsForm {
    public static function open(Player $sender): void  {
        $colors = ['Défaut', 'Rouge', 'Vert', 'Bleu', 'Jaune', 'Rose', 'Cyan', 'Blanc'];
        ...
        (new CustomForm('Paramètres'))
            ->toggle('guided_potions', 'Potions téléguidées', Manager::SETTINGS()->hasGuidedPotions($sender->getName()))
            ->toggle('scoreboard', 'Scoreboard', Manager::SETTINGS()->hasScoreboard($sender->getName()))
            ->toggle('combat_visibility', 'Visibilité en combat', Manager::SETTINGS()->hasCombatVisibility($sender->getName()))
            ...
            ->onSubmit(function (Player $player, array $data) use ($colorKeys): void {
                ...
                $player->sendMessage(Manager::BRANDING()->format('{prefix}{success}Vos paramètres ont bien été enregistrés.'));
            })
            ->sendToPlayer($sender);
    }
}
