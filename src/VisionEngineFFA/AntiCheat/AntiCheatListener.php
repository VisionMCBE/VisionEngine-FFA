...
final class AntiCheatListener implements Listener
{
    ...
    public function onBreak(BlockBreakEvent $event): void
    {
        ...
        if ($spread >= self::NUKER_SPREAD_LIMIT || $reach >= self::NUKER_REACH_LIMIT) {
            VisionAC::getInstance()->flag($player, 'Nuker', count($this->breaks[$key]) . ' blocs/s, portée ' . round($reach, 2));
        }
    }
    ...
    public function onQuit(PlayerQuitEvent $event): void
    {
        ...
        VisionAC::getInstance()->clear($event->getPlayer());
    }
    ...
    private function checkSpeed(Player $player, array &$st, int $tick, float $horiz): void
    {
        ...
        if ($horiz > $base) {
            ...
            if ($st['speedBuf'] > 0.8) {
                $st['speedBuf'] = 0.0;
                VisionAC::getInstance()->flag($player, 'Speed', 'vitesse ' . round($horiz, 3) . ' > ' . round($base, 3));
            }
        } else {
    ...
    private function checkFly(Player $player, array &$st, int $tick, float $dy): void
    {
        ...
        if ($st['air'] > self::FLY_GRACE && $dy > $expected + self::FLY_TOLERANCE) {
            ...
            if ($st['flyViol'] >= self::FLY_LIMIT) {
                $st['flyViol'] = 0.0;
                VisionAC::getInstance()->flag($player, 'Fly', 'dy ' . round($dy, 3) . ', attendu ' . round($expected, 3));
            }
        } else {
    ...
    private function checkPhase(Player $player, array &$st, int $tick, float $dx, float $dy, float $dz): void
    {
        ...
        if ($st['phaseBuf'] >= self::PHASE_BUFFER_LIMIT) {
            $st['phaseBuf'] = 0.0;
            VisionAC::getInstance()->flag($player, 'Phase', count($collisions) . ' collision(s) solide(s)');
        }
    }
    ...
    private function trackCps(Player $player): void
    {
        ...
        if (count($this->clicks[$key]) > self::CPS_MAX) {
            VisionAC::getInstance()->flag($player, 'CPS', count($this->clicks[$key]) . ' cps');
        }
    }
    ...
    private function checkReach(Player $damager, Entity $victim): void
    {
        ...
        if ($this->reachViolations[$key] >= self::REACH_BUFFER_LIMIT) {
            $this->reachViolations[$key] = 0;
            VisionAC::getInstance()->flag($damager, 'Reach', 'portée ' . round($distance, 2));
        }
    }
